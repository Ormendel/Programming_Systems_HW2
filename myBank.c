...
void D(int id_customer, double amount)
{
	...
	{
		printf("Customer #%d: adding %.2f to %.2f...\n",id_customer,amount,arr[id_customer-start].amount);
		arr[id_customer-start].amount+=amount;
		printf("Success!\n");
		B(id_customer);
	}			
}
...
void W(int id_customer, double amount)
{
	...
	{
		if(arr[id_customer-start].amount-amount<0)
	...
	else
	{
		...
		switch(check)
		{
			...
			case 1://Everything is ok, closing...
				arr[id_customer-start].status='N';
				arr[id_customer-start].amount=0;
